...
if st.button("🚀 判定開始"):
    ...
    if "1600m" in course:
        ...
        elif gate_num <= 4: score -= 5; minus_reasons.append("1600m内枠の砂被りリスク（-5）")
    else: # 1400m
    ...
    score += s_score; plus_reasons.append(f"脚質：{style}適性（{s_score:+d}）")
    ...
    score += jockey_scores.get(jockey, 10); plus_reasons.append(f"騎手：{jockey}（+{jockey_scores.get(jockey, 10)}）")
    
    # 血統評価
    if sire in ["シニスターミニスター", "ヘニーヒューズ"]: score += 20; plus_reasons.append(f"鉄板種牡馬：{sire}（+20）")
    elif sire in ["ドレフォン", "マインドユアビスケッツ"]: score += 15; plus_reasons.append(f"米国型：{sire}（+15）")
    elif sire in ["ゴールドドリーム", "シルバーステート", "ジャスタウェイ"]: score += 10; plus_reasons.append(f"ダート適性サンデー系：{sire}（+10）")
    elif sire == "ロードカナロア":
        if "1400m" in course and style in ["逃げ", "先行"]: score += 15; plus_reasons.append("ロードカナロア：1400m先行力評価（+15）")
        else: score += 5; plus_reasons.append("ロードカナロア：スピード適性（+5）")
    
    # 馬体重・前走成績
    if weight == "490kg以上": score += 5; plus_reasons.append("大型馬パワー評価（+5）")
    if prev_rank == "1着": score += 20; plus_reasons.append("前走1着（+20）")
